...
def weather(place):
    try:
        api_address = f'http://api.openweathermap.org/data/2.5/weather?appid=73f448c069df192842fc72a34f8ea837='
        word = place.split(' ')
        if len(word) == 1:
            city = word[0]
        else:
    ...
    while True:
        ...
        if message.strip()[0:7] == 'what is':
            try:
                ...
                print('Chatbot :', '{}? hmm.. I know what it is but I can not tell you'.format(message.strip()[7:]))
                chat_voice("{}? hmm.. I know what it is but I can not tell you".format(message.strip()[7:]))
        elif message.strip() == 'goodbye Rihanna':
            print('Chatbot : Bye {}'.format(name))
            chat_voice('Bye {}'.format(name))
            break
            ...
            print('Chatbot :', 'Search for {}'.format(message.strip()[5:]))
            chat_voice('Search for {}'.format(message.strip()[5:]))
            ...
            print('Chatbot :', reply)
            if str(reply)[:3] == '- -':
                chat_voice(str(reply)[3:])
            elif str(reply)[0] == '-':
                chat_voice(str(reply)[1:])
            else:
                chat_voice(reply)
...
def chat():
    while True:
        ...
        if message.strip()[0:7] == 'what is':
            ...
            print('Chatbot :', 'Searching for {}'.format(message.strip()[5:]))
            chat_voice('Searching for {}'.format(message.strip()[5:]))
            play_song(message.strip()[5:])
